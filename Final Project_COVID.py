...
def create_table_Colorado():
    ...
    count = 0
   ## relevant_data = cur.execute('SELECT date FROM Covid')
   ## print(relevant_data)
    ##for state_data in relevant_data:
     ##  print(state_data)
       ## lst.append(state_data[0])
    for state_data in response_2:
        if count ==25:
            break
        ##if state_data['submission_date'] in lst:
           ## continue
        if state_data['state'] == 'CO' and int(state_data['submission_date'][6]) > 2:
           # cur.execute("SELECT date from Dates WHERE date = ?, )
            try:
