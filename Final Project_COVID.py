...
## Grab data from the following api: https://data.cdc.gov/resource/9mfq-cb36.json?$$app_token=vUO9dTV5pir6UBXemfD6uL8lZ. After this, create a database and inside that database, create a table named Covid with the follpwing columns: date_id (integer), state (text), cases (integer), deaths (integer). 
##Create a table named Dates with the following columns: date (text unique), date_id (Integer, primary key). Grab the Covid data (only Colorado) from the API and insert it into the table.
...
def create_table_Colorado():
    ...
    for state_data in response_2:
        if count ==25:
            break
        #We used Colorado (CO), not Montana
        if state_data['state'] == 'MO' and int(state_data['submission_date'][6]) > 2:
            try:
